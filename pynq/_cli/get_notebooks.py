...
def _get_notebooks_parser():
    ...
    parser = _GetNotebooksParser(description="Deliver available PYNQ " "notebooks")
    ...
    group.add_argument(
        "-d", "--device", type=str, help="Provide a specific device name (XRT shell)"
    )
    group.add_argument(
        "-i",
        "--interactive",
    ...
    parser.add_argument(
        "--from-package", type=str, help="Get notebooks only from target package name"
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Do not produce logging output"
    )
    return parser
...
def main():
    ...
    if args.device:
        ...
        while idx < 0 or idx >= shells_num:
            idx = input(
                "Select for which shell you want to deliver the "
    ...
    if args.notebooks:
        if "all" in args.notebooks:
            if len(args.notebooks) > 1:
                raise ValueError(
                    "The special keyword 'all' cannot be used "
            ...
            if not_found:
                raise ValueError(
                    "Notebooks modules '{}' not found. Make "
        ...
        if from_package:
            ...
            if not nbs_printable:
                logger.warn(
                    "No notebooks available for package '{}', nothing "
        ...
        if not args.force:
            ...
            nbs_installed_printable = [
                nb for nb in nbs_printable if nb in nbs_installed_all
            ]
            nbs_printable = [nb for nb in nbs_printable if nb not in nbs_installed_all]
            if nbs_installed_printable:
                print(
                    "Already delivered notebooks (use --force to force "
                    "delivery):\n- {}".format("\n- ".join(nbs_installed_printable))
                )
        if not nbs_printable:
        ...
        while True:
            ...
            if coiche in no:
                return
            coiche = input(
                "Please respond with 'yes' or 'no' (or 'y' or "
    ...
    try:
        ...
        for ext in notebooks_ext_man.list:
            if (
                args.notebooks
                and "all" not in [n.lower() for n in args.notebooks]
                and ext.name.lower() not in [n.lower() for n in args.notebooks]
            ):
                continue
            if from_package and from_package != ext.module_name.split(".")[0].lower():
                continue
            if ext.module_name in _get_installed(delivery_path) and not args.force:
                if args.notebooks:
                    logger.info(
                        "Notebooks '{}' are already delivered and "
                        "will be ignored, use --force to "
                        "override.".format(ext.module_name)
                    )
                continue
            logger.info(
                "Delivering notebooks '{}'...".format(
                    os.path.join(delivery_path, ext.name)
                )
            )
            ext_mod = ext.load()
            if (
                hasattr(ext_mod, "__no_root__")
                and type(ext_mod.__no_root__) is bool
                ...
                folder = False
                logger.info(
                    "Folder '{}' will not be created. Notebooks will "
            ...
            if hasattr(ext_mod, "deliver_notebooks"):
                ...
                # interface of `pynqutils.setup_utils:deliver_notebooks`, and must
                # implement the same behavior. It can be used to implement some
                # pre- or post-processing calling
                # `pynqutils.setup_utils:deliver_notebooks` inside, or an entirely custom
                # delivery procedure
        ...
        if not os.path.isdir(delivery_path):
            logger.warn(
                "No notebooks available for target device, nothing " "will be delivered"
            )
