...
class AudioDirect(DefaultIP):
    ...
    def __init__(self, description, gpio_name=None):
        ...
        self._ffi.cdef(
            """unsigned int Xil_Out32(unsigned int Addr, 
                                                 unsigned int Value);"""
        )
        self._ffi.cdef("""unsigned int Xil_In32(unsigned int Addr);""")
        self._ffi.cdef(
            """void record(unsigned int BaseAddr, 
                                      unsigned int * BufAddr, 
                                      unsigned int Num_Samles_32Bit);"""
        )
        self._ffi.cdef(
            """void play(unsigned int BaseAddr, 
                                    unsigned int * BufAddr, 
                                    unsigned int Num_Samles_32Bit);"""
        )
    ...
    def load(self, file):
        ...
        with wave.open(file_abs, "rb") as pdm_file:
            temp_buffer = numpy.fromstring(
                pdm_file.readframes(pdm_file.getnframes()), dtype="<u2"
            )
...
class AudioADAU1761(DefaultIP):
    ...
    def __init__(self, description):
        ...
        self._ffi.cdef(
            """void bypass(unsigned int audio_mmap_size,
                          unsigned int nsamples, unsigned int volume,
                          int uio_index, int iic_index) ;"""
        )
        self._ffi.cdef(
            """void record(unsigned int audio_mmap_size,
    ...
    def configure(self, sample_rate=48000, iic_index=1, uio_name="audio-codec-ctrl"):
