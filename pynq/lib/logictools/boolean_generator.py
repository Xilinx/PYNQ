...
class BooleanGenerator:
    ...
    def __init__(self, mb_info, intf_spec_name="PYNQZ1_LOGICTOOLS_SPECIFICATION"):
    ...
    def trace(self, use_analyzer=True, num_analyzer_samples=DEFAULT_NUM_TRACE_SAMPLES):
        ...
        """
        if use_analyzer:
            self.analyzer = TraceAnalyzer(self._mb_info, intf_spec_name=self.intf_spec)
    ...
    def setup(self, expressions, frequency_mhz=DEFAULT_CLOCK_FREQUENCY_MHZ):
        ...
        """
        try:
            from pyeda.inter import expr2truthtable, exprvar
        except ImportError:
        ...
        if (
            not 1
            <= len(expressions)
            <= self.intf_spec["interface_width"]
        ...
        if isinstance(expressions, list):
            for i in range(len(expressions)):
                self.expressions["Boolean expression {}".format(i)] = deepcopy(
                    expressions[i]
                )
        ...
        mailbox_addr = self.logictools_controller.mmio.base_addr + MAILBOX_OFFSET
        mailbox_regs = [
            Register(addr) for addr in range(mailbox_addr, mailbox_addr + 4 * 64, 4)
        ]
        ...
        for expr_label, expression in self.expressions.items():
            ...
            if "=" not in expression:
                raise ValueError("Boolean expression must have form Output = Function.")
            ...
            if expr_out in self.output_pins:
                raise ValueError(
                    "The same output pin should not be driven by "
            ...
            if expr_out in self.intf_spec["traceable_io_pins"]:
                ...
                output_pin_num = self.intf_spec["non_traceable_outputs"][expr_out]
            else:
            ...
            for orig_name, p_name in zip(
                input_pins_with_dontcares, ["p{}".format(i) for i in range(5)]
            ):
            ...
            self.logictools_controller.write_command(CMD_READ_BOOLEAN_DIRECTION)
            ...
            for i in range(5):
                ...
                if truth_table_inputs[i] in unique_input_pins:
                    if (
                        truth_table_inputs[i] in self.intf_spec["traceable_io_pins"]
                        and truth_table_inputs[i] in self.intf_spec["traceable_io_pins"]
                    ):
                        input_pin_ix = self.intf_spec["traceable_io_pins"][
                            truth_table_inputs[i]
                        ]
                    elif (
                        truth_table_inputs[i] in self.intf_spec["non_traceable_inputs"]
                    ):
                        ...
                        raise ValueError(
                            "Invalid input pin " "{}.".format(truth_table_inputs[i])
                        )
                else:
                    input_pin_ix = 0x1F
            ...
            for pin_name in unique_input_pins:
                if pin_name in self.intf_spec["traceable_io_pins"]:
                    ...
                    waveform_dict["signal"][0].append(
                        {"name": pin_name, "pin": pin_name}
                    )
            if not stimulus_traced:
                del waveform_dict["signal"][0]

            if expr_out in self.intf_spec["traceable_io_pins"]:
                waveform_dict["signal"][-1].append({"name": expr_out, "pin": expr_out})
            else:
    ...
    def connect(self):
        ...
        ioswitch_pins = list()
        ioswitch_pins += [
            self.intf_spec["traceable_io_pins"][input_pin]
            for input_pin in self.input_pins
            if input_pin in self.intf_spec["traceable_io_pins"]
        ]
        ioswitch_pins += [
            self.intf_spec["traceable_io_pins"][output_pin]
        ...
        self.logictools_controller.config_ioswitch(
            ioswitch_pins, IOSWITCH_BOOLEAN_SELECT
        )
    ...
    def disconnect(self):
        ...
        # Send list to Microblaze processor for handling
        self.logictools_controller.config_ioswitch(ioswitch_pins, IOSWITCH_DISCONNECT)
    ...
    def run(self):
        ...
        """
        if self.logictools_controller.status[self.__class__.__name__] == "RESET":
            raise ValueError("Generator must be at least READY before RUNNING.")
        self.connect()
        self.logictools_controller.steps = 0
    ...
    def analyze(self):
        ...
        """
        if self.analyzer is not None:
            analysis_group = self.analyzer.analyze(self.logictools_controller.steps)
            for expr_label in self.expressions.keys():
                if self.logictools_controller.steps:
                    self.waveforms[expr_label].append("stimulus", analysis_group)
                    self.waveforms[expr_label].append("analysis", analysis_group)
                else:
            ...
            if 0 < self.logictools_controller.steps < 3:
                for key in self.waveforms[expr_label].waveform_dict:
                    for annotation in ["tick", "tock"]:
                        if annotation in self.waveforms[expr_label].waveform_dict[key]:
                            del self.waveforms[expr_label].waveform_dict[key][
                                annotation
                            ]
            else:
