...
These projects are considered SDK *Application* projects and contain the top level application. IP related software libraries are included in a Board Support Package (BSP) project, and are linked to from the Application project. 
...
Note that each MicroBlaze has the same address space. However, the ARM Equivalent Address Space are different for each IOP. Any binary compiled for one Pmod IOP will work on the second Pmod IOP in the overlay as the MicroBlaze address space is identical, however, it may not run on Arduino IOP since the MicroBlaze address space do slightly differ. 
