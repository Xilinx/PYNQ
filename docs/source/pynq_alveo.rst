...
an optional keyword parameter ``waitlist`` that can be used to create a
...
between accelerator invokations. The ``waitlist`` is a list of wait handles
...
.. code:: python
    ...
    ol.vadd_1.call(input3, output, output, waitlist=(handle,))
